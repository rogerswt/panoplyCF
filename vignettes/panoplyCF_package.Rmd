...
* it's re-useable.  New dat can be inserted into an existing embedding via the CF bins.
...
# gate the data.
pan_params = flowCore::colnames(ff)[c(7:9, 11:22)]
pan_params

# do the panoplyCF computation (takes a couple of minutes)
pan = panoply(fcs = ff, parameters = pan_params, nclust = 30)
...
```{r, out.width="100%", fig.height=14, fig.width=14}

decorate_sample_panoply(fcs = ff, panoply_obj = pan, colorscale = FALSE)
```

## Remarks
Please note that the package version 0.1.0 is alpha.  It's likely to break.  Please let 
